...
ggplot(fiskar1, aes(x=le, y=osl)) + geom_point() + labs(
  ...
  y = "Þyngd") 
```
Við sjáum að þyngd vex með lengd og virðast fylgja veldisvexti. Við getum því ályktað að þyngd aukist í hlutfalli við veldi af lengd þ.e. að til séu fastar $\alpha$ og $c$ þannig að $w = \alpha l^{c}$ þar sem $w$ er þyngd og $l$ er lengd. Það er því eðlilegt að líta á stærðirnar á log-log grafi og fá þannig línulegt samband $log(w) = clog(l)+log(\alpha)$.
