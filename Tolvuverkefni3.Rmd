...
Búum nú til tvær nýjar breytur og látum báðar vera flokkabreytur. Búum fyrst til flokkabreytuna hafsvaedi sem hefur gildin "SV", "SA", "NV" eða "NAu" og hvert svæði er skilgreint út frá lengdar- og breiddargráðu sem við sækjum úr breytunni reit, þ.e. ef svæði hefur breiddargráðu minnni en 65 þá erum við fyrir sunnan annars fyrir norðan og ef svæði hefur lengdargráðu minni en -19 þá erum við fyrir vestan annars fyrir austan. Síðan búum við til flokkabreytuna kt2 sem segir til um hvort fiskur sé kynþrosk eða ókynþroska.
...
taflab <- ah %>%
  ...
  count(name = "Fjöldi")
taflab %>%
  kable(align = "lll", caption="Tafla 1: Sýnir fjölda fiska af hvoru kynþroskastigi eftir hafsvæðum.") %>%
...
taflab[ , "Hlutfall"] <- f
taflab %>%
  kable(align = "llll", caption="Tafla 2: Sýnir fjölda og hlutfall fiska af hvoru kynþroskastigi eftir hafsvæðum.") %>%
  kable_styling(position = "center")
```
...
ggplot(ah, aes(hafsvaedi, fill=kt2)) + 
  ...
  xlab("Hafsvæði") + 
  ylab("Fjöldi") +
  labs(fill = "Þroskastig", caption = "Mynd 1: Mynd sem er lýsandi fyrir fjölda fiska af hvoru kynþroskastigi í hverjum flokki af hafsvæðunum fjórum.", Position = "center") +
...
Teikum næst mynd sem sýnir lengd fiska eftir aldri þegar litið er á aldur sem samfellda breytu ásamt meðallengd fiska fyrir hvern aldurshóp sem stærri rauða punkta.
```{r, Mynd sem sýnir lengd fiska eftir aldri}
  ggplot(ah, aes(x = aldur, y = le)) +
    labs(
        title = "Graf sem sýnir lengd fiska eftir aldri, þar sem aldur er samfelld breyta.",
        x = "Aldur",
        y = "Lengd"
        ) +
...
```{r, Kassarit sem sýnir lengd fiska eftir aldri}
  ...
  labs(
      title = "Graf sem sýnir lengd fiska eftir aldri, þar sem aldur er strjál breyta.",
      x = "Aldur",
      y = "Lengd"
      ) +
  geom_boxplot()
...
fiskar # ATHUGA HVORT VIÐ VILJUM SÝNA TÖFLUNA
```
...
ggplot(ah_long, aes(le)) +  geom_histogram(binwidth = 3) +
   geom_line(data = normaldens, aes(x = le, y = normal_curve),color="red") + xlab("Lengd") + ylab("Fjöldi") + facet_wrap(vars(hafsvaedi)) + labs(caption = "Mynd 2: Fjögur stuðlarit af lengd fiska fyrir hvert hafsvæði.") + theme(plot.caption = element_text(hjust = 0.5))
```
...
Við fáum sem sagt að p-gildi slembivalsstærðarinnar er $p = 0.6598$. Í e-lið fékkst p-gildið $p = 0.672$ úr t-prófinu og því ekki um mikinn mun að ræða. Líkt og í e-lið þá höfnum við ekki $H_{0}$ þar sem p-gildið er hærra en $\alpha = 0.05$. Það er því ekki marktækur munur á meðallengd fiska á hafsvæðunum tveimur. 
...
Við athugum nú hvort marktækur munur sé á hlutfalli kynþroska fiska á hafsvæðunum tveimur, $ \rho_{NV}$ og $\rho_{NAu}$. Ef marktækur munur er á hlutfalli kynþroska fiska á hafsvæðunum tveimur myndum við þurfa að hafna tilgátuprófinu:
...
Niðurstöður úr prófinu gefa að prófstærðin er $0$ og p-gildið er $1$. Við höfnum ekki $H_{0}$ þar sem p-gildið er hærra en $\alpha$. Það er því ekki marktækur munur á fjölda kynþroska fiska á hafsvæðunum tveimur. Þá fáum við að $[-0.1796337, 0.1796337]$ er $95\%$ öryggisbil fyrir fjölda kynþroska fiska á hafsvæðunum tveimur. Við sjáum að matið sjálft á hlutfalli kynþroska fiska á hafsvæðunum tveimur er $0.3$ fyrir hvort hafsvæði. Við athugum sérstaklega að öryggisbilið inniheldur $0$ en það stafar af því að við getum ekki hafnað núlltilgátunni. Það er sem sagt möguleiki á því að mismunur meðaltalanna sé 0.

## i) liður
```{r, þyngd sem fall af lengd}
#Veljum annað hafsvæðanna úr e-lið, hér veljum við hafsvæðið NV
ggplot(fiskar1, aes(x=le, y=osl)) + geom_point() + labs(
  title="Samband lengdar og þyngdar fiska á hafsvæðinu NV",
  x = "Lengd", 
  y = "Þyngd") 
```

Við sjáum að þyngd vex með lengd og virðast fylgja veldisvexti. Við getum því ályktað að þyngd aukist í hlutfalli við veldi af lengd þ.e. að til séu fastar $\alpha$ og $c$ þannig að $w = \alpha l^{c}$ þar sem $w$ er þyngd og $l$ er lengd. Það er því eðlilegt að líta á stærðirnar á log-log grafi og fá þannig línulegt samband $ln(w) = cln(l)+ln(\alpha)$.
```{r, log-log graf þyngd sem fall af lengd}
#Teiknum einnig mynd sem sýnir samband stærðanna á log-log grafi
ggplot(fiskar1, aes(x=log(le), y=log(osl))) + geom_point() + labs(
...
Línulega aðhvarfsgreiningarlíkanið er því $w = 3.0199l - 4.7949$. Þá fáum við að skýringarhlutfallið er $R^2 = 0.983$ svo ætla má að samband frum- og fylgibreytu sé næstum línulegt og líkanið gefi traustverðuga spá um þyngd út frá lengd.
...
## k) liður
Teiknum kasssarit sem sýnir lengd á móti aldri fyrir hafsvæðið NAu
```{r, kassarit lengd á móti aldri fyrir hafsvæðið NAu}
#Teiknum kasssarit sem sýnir lengd á móti aldri
ggplot(data = fiskar2,aes(x = factor(aldur), y = le)) + geom_boxplot() + labs(
        x = "Aldur",
        y = "Lengd"
    )
```
...
litid <- lm(le~aldur, data=fiskar2)
stort <- lm(le~factor(aldur), data=fiskar2)
anova(litid, stort)
```

Við sjáum af kassaritinu að ofan að almennt fer lengd fiska vaxandi með hækkandi aldri. Dreifni í lengd innan mismunandi aldurshópa er nokkurn veginn jöfn fyrir aldurshópana $4, 5$ og $6$ en minni fyrir þá aldurshópa eldri fiska enda um mun færri fiska í hverjum hópi að ræða í þeim aldurshópum. Við viljum athuga hvort meðaltal lengda hinna mismunandi aldurshópa séu mismunandi. Af myndinni að ofan sjáum við að svo ætti líklegast að vera en við ætlum nú að athuga hvort svo sé með einþátta fervikagreiningu. Við athugum nú hvort marktækur munur, með $95\%$ marktæknikröfu, sé á meðallengd fiska eftir aldurshópum. Látum $\mu_{i}$ tákna meðaltal lengda fiska í aldurshópi $i$. Við viljum kanna tilgátuna

$$H_{0}: \mu_{3} = \mu_{4} = ... = \mu_{10}\\$$

Á móti gagntilgátunni $H_{1}:$ Að minsta kosti eitt meðaltal er frábrugðið hinum. Niðurstöður úr einþátta fervikagreiningunni gefa okkur p-gildið $p = 0.003151$ sem er lægra en $\alpha = 0.05$ og því höfnum við núlltilgátunni. Niðurstaðan er því sú að minnsta kosti eitt meðaltal er frábrugðið hinum.
...
kort <- ggplot() +
  ...
  geom_polygon(data=Iceland, aes(x=long, y=lat, group=group), color="black", fill="blue") + 
  coord_quickmap(xlim = xlim, ylim = ylim) +
  xlab("Lengdargráða") + ylab("Breiddargráða")
kort
```

Bætum síðan inn tilkynningarskyldureitunum frá okkar hafssvæðum:

```{r, tilkynningarskildureitir}
colorNV <- filter(ah, hafsvaedi == "NV")$hafsvaedi
colorNAu <- filter(ah, hafsvaedi =="NAu")$hafsvaedi
...
kort +
  geom_label(aes(x=xNV, y=yNV, label=as.character(reitirNV), fill="NV"), size=2) +
  geom_label(aes(x=xNAu, y=yNAu, label=as.character(reitirNAu), fill="NAu"), size=2) +
  labs(fill="Hafsvæði")
```
